...
export function Select({ options, alternativeTitle, title, Icon, defaultValue, handleSelectDatabase }: Select) {
    ...
    return (
        <div className="w-full h-full overflow-hidden bg-light dark:bg-gray border border-primary-100 font-bold rounded-lg flex gap-2 px-2 justify-center items-center">
            {Icon && <Icon className="text-2xl fill-primary-100 dark:fill-light" />}
            <select
                className="w-full h-full p-2 bg-light dark:bg-gray text-primary-100 dark:text-light cursor-pointer outline-none "
